{{-- resources/views/backend/doctors/_form.blade.php OR your create/edit.blade.php --}}
<form action="{{ $action }}" method="POST" enctype="multipart/form-data">
    ...
    <div class="row">
        ...
        <div class="col-md-4 mb-3">
            ...
            @error('registration_council') <div class="text-danger small">{{ $message }}</div> @enderror
        </div>


        <!-- DEGREES: FULLY WORKING (Create + Edit) -->
        <div class="col-md-6 mt-3">
            ...
            <div class="dropdown w-100" data-bs-auto-close="outside">
                <button class="btn btn-outline-secondary dropdown-toggle w-100 text-start" 
                        type="button" id="degreeDropdown" data-bs-toggle="dropdown">
        ...
        <div class="col-md-6 mb-3">
            ...
            @error('languages') <div class="text-danger small">{{ $message }}</div> @enderror
        </div>
        ...
        <div class="col-md-6 mb-3">
            ...
            @error('medical_service_sub_category_id') <div class="text-danger small">{{ $message }}</div> @enderror
        </div>
        ...
        <!-- TIME SLOTS – 100% WORKING (even with old "evening" format) -->
        <div class="col-md-12 mb-4">
            ...
            @php
                ...
                if (!empty($rawSlots)) {
                    if (isset($rawSlots['evening']) || isset($rawSlots['morning'])) {
                        foreach ($rawSlots as $val) {
                ...
                if (empty($slots)) $slots = [['start' => '', 'end' => '']];
            @endphp

            <div id="timeSlotsContainer">
                @foreach($slots as $index => $slot)
                    <div class="row g-3 align-items-center mb-2 time-slot">
                        <div class="col-md-5">
            ...
            @error('slots') <div class="text-danger small">{{ $message }}</div> @enderror
        </div>
        ...
        <div class="col-md-6 mb-3">
            ...
            @if($doctor?->profile_image)
                <img src="{{ $doctor->profile_image_url }}" class="img-thumbnail mt-2" width="100" alt="Profile">
            @endif
        </div>

        <div class="col-md-6 mb-3">
            <label class="form-label">Short Video (Optional)</label>
            <input type="file" name="short_video" class="form-control" accept="video/*">
            @if($doctor?->short_video)
                <video width="200" controls class="mt-2 rounded">
        ...
        <div class="col-12 text-end mt-4">
            <a href="{{ route('admin.doctors.index') }}" class="btn btn-secondary me-3">Cancel</a>
            <button type="submit" class="btn btn-primary px-5">{{ $buttonText }}</button>
        </div>
    </div>
</form>

<!-- JAVASCRIPT: Degrees Search + Time Slots -->
<script>
    // Degrees: Live Search
    document.getElementById('degreeSearch')?.addEventListener('keyup', function () {
        const term = this.value.toLowerCase();
        document.querySelectorAll('#degreeList label').forEach(label => {
            label.style.display = label.textContent.toLowerCase().includes(term) ? '' : 'none';
        });
    });

    // Degrees: Update button text on selection
    document.addEventListener('change', function (e) {
        if (e.target.name === 'degree_id[]') {
            const count = document.querySelectorAll('input[name="degree_id[]"]:checked').length;
            document.getElementById('degreeDropdownLabel').textContent =
                count > 0 ? count + ' degree(s) selected' : 'Choose Degrees';
        }
    });

    // Time Slots: Dynamic Add/Remove
