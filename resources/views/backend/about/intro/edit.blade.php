                            ...
                            <div class="tab-content" id="wizard-tabContent">
                                <div class="tab-pane fade show active" id="wizard-contact" role="tabpanel" aria-labelledby="wizard-contact-tab">
                                    ...
                                    <form class="row g-3 needs-validation custom-input"
                                        novalidate
                                        action="{{ route('admin.manage-about-intro.update',$about->id) }}"
                                        method="POST"
                                        ...
                                        <div class="col-md-6 mt-5">
                                            ...
                                            <!-- Existing Images -->
                                            <div class="d-flex flex-wrap mt-3" id="existingImages">

                                                @if($images)
                                                @foreach($images as $img)

                                                <div class="preview-box existing-box" data-image="{{ $img }}">
                                                    <img src="{{ asset('uploads/about/'.$img) }}" width="80">

                                                    <span class="remove-btn" onclick="removeExistingImage(this)">×</span>
                                                </div>
                                        ...
                                        <div class="col-md-12">
                                            ...
                                            <textarea class="form-control"
                                                    id="editor"
                                                    name="desc"
                                                    required>{{ old('desc',$about->desc) }}</textarea>
                                        </div>
                                        ...
                                        <div class="col-md-12 mt-5">
                                            ...
                                            <textarea class="form-control" id="note"
                                                    name="note">{{ old('note',$about->note) }}</textarea>
                                        </div>

                                        <!-- Buttons -->
                                        <div class="col-12 text-end">
                                            <a href="{{ route('admin.manage-about-intro.index') }}" class="btn btn-danger">
                                                Cancel
                                            </a>
...
function removeExistingImage(el){
    ...
    let box = el.closest('.existing-box');
    let filename = box.dataset.image;

    removedExisting.push(filename);

    document.getElementById('removedImages').value = JSON.stringify(removedExisting);
